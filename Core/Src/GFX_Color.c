...
void GFX_DrawLine(int x_start, int y_start, int x_end, int y_end, ILI_Color color)
{
	if(x_start == x_end){
...
#endif
#ifdef CIRCLE_HELPER
void GFX_DrawCircleHelper( int x0, int y0, uint16_t r, uint8_t cornername, ILI_Color color)
{
    int16_t f     = 1 - r;
    int16_t ddF_x = 1;
    int16_t ddF_y = -2 * r;
...
void GFX_DrawFillCircleHelper(int x0, int y0, uint16_t r, uint8_t cornername, int16_t delta, ILI_Color color)
{
...
void GFX_DrawFillCircle(int x0, int y0, uint16_t r, ILI_Color color)
{
...
void GFX_DrawRoundRectangle(int x, int y, uint16_t w, uint16_t h, uint16_t r, ILI_Color color)
{
...
void GFX_DrawFillRoundRectangle(int x, int y, uint16_t w, uint16_t h, uint16_t r, ILI_Color color)
{
    // smarter version
...
#endif
#if USING_TRIANGLE == 1
void GFX_DrawTriangle(int x0, int y0, int x1, int y1, int x2, int y2, ILI_Color color)
{
...
void GFX_DrawFillTriangle(int x0, int y0, int x1, int y1, int x2, int y2, ILI_Color color)
{
...
#if USING_IMAGE == 1
#if AVR_USING == 1
void GFX_Image_P(int x, int y, uint8_t *img, uint8_t w, uint8_t h, ILI_Color color)
{
...
void GFX_ImageRotate(int x, int y, const uint8_t *img, uint8_t w, uint8_t h, ILI_Color color, uint16_t angle)
{
	angle %= 360;
